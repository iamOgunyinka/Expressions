...
namespace Expression
{
    ...
    struct Symbol
    {
        explicit Symbol( std::string && lex ): m_lexeme( std::move( lex ) ) { }
        explicit Symbol( char const *lex ): m_lexeme( lex ) { }
    ...
    public:
        ...
        token_type get_negated_constant_or_unary_minus()
        {
            ...
            if( is_numeric_constant{} ( current_character ) ){
                ...
                return { Symbol( str_buf ), expression_type::e_constant };
            } else {
                return { Symbol( str_buf ), expression_type::e_binary_operator };
            }
        }
        ...
        token_type get_variable_token()
        {
            ...
            update_current_token();
            return { Symbol { str_buf }, expression_type::e_variable };
        }
        ...
        token_type get_token()
        {
            for( ; ; ){
                switch( current_character ){
                    case EOF_F:
                        update_current_token();
                        return { Symbol {}, expression_type::e_none };
                    case '|': case ' ':
                        update_current_token();
                        break;
                    case '*':
                        update_current_token();
                        return { Symbol { "*" }, expression_type::e_binary_operator };
                    case '/':
                        update_current_token();
                        return { Symbol { "/" }, expression_type::e_binary_operator };
                    case '+':
